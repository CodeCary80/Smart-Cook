...
var VERSION = "0.27.3";
//#endregion
//#region node_modules/@anthropic-ai/sdk/_shims/registry.mjs
var auto = false;
var kind = void 0;
var fetch$1 = void 0;
var File$1 = void 0;
...
function setShims(shims, options = { auto: false }) {
	if (auto) throw new Error(`you must \`import '@anthropic-ai/sdk/shims/${shims.kind}'\` before importing anything else from @anthropic-ai/sdk`);
	if (kind) throw new Error(`can't \`import '@anthropic-ai/sdk/shims/${shims.kind}'\` after \`import '@anthropic-ai/sdk/shims/${kind}'\``);
	auto = options.auto;
	kind = shims.kind;
	fetch$1 = shims.fetch;
	shims.Request;
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/_shims/MultipartBody.mjs
/**
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/_shims/web-runtime.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/_shims/index.mjs
/**
* Disclaimer: modules in _shims aren't intended to be imported by SDK users.
*/
if (!kind) setShims(getRuntime(), { auto: true });
//#endregion
//#region node_modules/@anthropic-ai/sdk/streaming.mjs
		...
		async function* iterator() {
			...
			try {
				for await (const sse of _iterSSEMessages(response, controller)) {
					...
					if (sse.event === "error") throw APIError$1.generate(void 0, `SSE Error: ${sse.data}`, sse.data, createResponseHeaders(response.headers));
				}
				done = true;
			} catch (e) {
...
async function* _iterSSEMessages(response, controller) {
	if (!response.body) {
		controller.abort();
		throw new AnthropicError$1(`Attempted to iterate over a response with no body`);
	}
...
var LineDecoder = class LineDecoder {
	...
	decodeText(bytes) {
		...
		if (typeof Buffer !== "undefined") {
			...
			throw new AnthropicError$1(`Unexpected: received non-Uint8Array (${bytes.constructor.name}) stream chunk in an environment with a global "Buffer" defined, which this library assumes to be Node. Please report this error.`);
		}
		if (typeof TextDecoder !== "undefined") {
			...
			throw new AnthropicError$1(`Unexpected: received non-Uint8Array/ArrayBuffer (${bytes.constructor.name}) in a web platform. Please report this error.`);
		}
		throw new AnthropicError$1(`Unexpected: neither Buffer nor TextDecoder are available as globals. Please report this error.`);
	}
	flush() {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/uploads.mjs
...
async function toFile$1(value, name, options) {
	...
	if (isResponseLike(value)) {
		...
		return new File$1(isBlobLike(blob) ? [await blob.arrayBuffer()] : [blob], name, options);
	}
...
var APIClient = class {
	constructor({ baseURL, maxRetries = 2, timeout = 6e5, httpAgent, fetch: overridenFetch }) {
	...
	*/
	...
	stringifyQuery(query) {
		return Object.entries(query).filter(([_, value]) => typeof value !== "undefined").map(([key, value]) => {
			...
			throw new AnthropicError$1(`Cannot stringify type ${typeof value}; Expected string, number, boolean, or null. If you need to pass nested query parameters, you can manually encode them, e.g. { query: { 'foo[key1]': value1, 'foo[key2]': value2 } }, and please open a GitHub issue requesting better support for your use case.`);
		}).join("&");
	}
...
var validatePositiveInteger = (name, n) => {
	if (typeof n !== "number" || !Number.isInteger(n)) throw new AnthropicError$1(`${name} must be an integer`);
	if (n < 0) throw new AnthropicError$1(`${name} must be a positive integer`);
	return n;
};
var castToError = (err) => {
...
function debug(action, ...args) {
	if (typeof process !== "undefined" && process?.env?.["DEBUG"] === "true") console.log(`Anthropic:DEBUG:${action}`, ...args);
}
/**
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/error.mjs
var error_exports = /* @__PURE__ */ __exportAll({
	...
	AnthropicError: () => AnthropicError$1,
...
var AnthropicError$1 = class extends Error {};
var APIError$1 = class APIError$1 extends AnthropicError$1 {
	...
	static generate(status, errorResponse, message, headers) {
		if (!status) return new APIConnectionError$1({
			message,
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resource.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/PromptCachingBetaMessageStream.mjs
...
var PromptCachingBetaMessageStream = class PromptCachingBetaMessageStream {
	constructor() {
		...
		_PromptCachingBetaMessageStream_handleError.set(this, (error) => {
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_errored, true, "f");
			if (error instanceof Error && error.name === "AbortError") error = new APIUserAbortError$1();
			if (error instanceof APIUserAbortError$1) {
				__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_aborted, true, "f");
				return this._emit("abort", error);
			}
			if (error instanceof AnthropicError$1) return this._emit("error", error);
			if (error instanceof Error) {
				const anthropicError = new AnthropicError$1(error.message);
				anthropicError.cause = error;
				return this._emit("error", anthropicError);
			}
			return this._emit("error", new AnthropicError$1(String(error)));
		});
		__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_connectedPromise, new Promise((resolve, reject) => {
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_resolveConnectedPromise, resolve, "f");
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_rejectConnectedPromise, reject, "f");
		}), "f");
		__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_endPromise, new Promise((resolve, reject) => {
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_resolveEndPromise, resolve, "f");
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_rejectEndPromise, reject, "f");
		}), "f");
		__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_connectedPromise, "f").catch(() => {});
		__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_endPromise, "f").catch(() => {});
	}
	/**
	...
	*/
	...
	static createMessage(messages, params, options) {
		const runner = new PromptCachingBetaMessageStream();
		for (const message of params.messages) runner._addPromptCachingBetaMessageParam(message);
	...
	_run(executor) {
		executor().then(() => {
			this._emitFinal();
			this._emit("end");
		}, __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_handleError, "f"));
	}
	...
	async _createPromptCachingBetaMessage(messages, params, options) {
		...
		__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_beginRequest).call(this);
		...
		this._connected();
		for await (const event of stream) __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_addStreamEvent).call(this, event);
		if (stream.controller.signal?.aborted) throw new APIUserAbortError$1();
		__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_endRequest).call(this);
	}
	_connected() {
		if (this.ended) return;
		__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_resolveConnectedPromise, "f").call(this);
		this._emit("connect");
	}
	get ended() {
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_ended, "f");
	}
	get errored() {
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_errored, "f");
	}
	get aborted() {
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_aborted, "f");
	}
	abort() {
	...
	*/
	on(event, listener) {
		(__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event] || (__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = [])).push({ listener });
		return this;
	}
	/**
	...
	*/
	off(event, listener) {
		const listeners = __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event];
	...
	*/
	once(event, listener) {
		(__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event] || (__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = [])).push({
			listener,
			once: true
		});
		return this;
	...
	*/
	emitted(event) {
		return new Promise((resolve, reject) => {
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, true, "f");
	...
	async done() {
		__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, true, "f");
		await __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_endPromise, "f");
	}
	get currentMessage() {
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
	}
	/**
	...
	*/
	async finalMessage() {
		await this.done();
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalMessage).call(this);
	}
	/**
	...
	*/
	async finalText() {
		await this.done();
		return __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalText).call(this);
	}
	_emit(event, ...args) {
		if (__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_ended, "f")) return;
		if (event === "end") {
			__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_ended, true, "f");
			__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_resolveEndPromise, "f").call(this);
		}
		const listeners = __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event];
		if (listeners) {
			__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = listeners.filter((l) => !l.once);
			listeners.forEach(({ listener }) => listener(...args));
		}
		if (event === "abort") {
			const error = args[0];
			if (!__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, "f") && !listeners?.length) Promise.reject(error);
			__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_rejectConnectedPromise, "f").call(this, error);
			__classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_rejectEndPromise, "f").call(this, error);
			this._emit("end");
			return;
		}
		if (event === "error") {
	...
	_emitFinal() {
		if (this.receivedMessages.at(-1)) this._emit("finalPromptCachingBetaMessage", __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalMessage).call(this));
	}
	...
	[(_PromptCachingBetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _PromptCachingBetaMessageStream_getFinalMessage = function _PromptCachingBetaMessageStream_getFinalMessage() {
		if (this.receivedMessages.length === 0) throw new AnthropicError$1("stream ended without producing a PromptCachingBetaMessage with role=assistant");
		...
		if (textBlocks.length === 0) throw new AnthropicError$1("stream ended without producing a content block with type=text");
		return textBlocks.join(" ");
	}, _PromptCachingBetaMessageStream_beginRequest = function _PromptCachingBetaMessageStream_beginRequest() {
		if (this.ended) return;
		__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, void 0, "f");
	}, _PromptCachingBetaMessageStream_addStreamEvent = function _PromptCachingBetaMessageStream_addStreamEvent(event) {
		if (this.ended) return;
		const messageSnapshot = __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_accumulateMessage).call(this, event);
		this._emit("streamEvent", event, messageSnapshot);
		switch (event.type) {
			case "content_block_delta": {
				const content = messageSnapshot.content.at(-1);
				if (event.delta.type === "text_delta" && content.type === "text") this._emit("text", event.delta.text, content.text || "");
				else if (event.delta.type === "input_json_delta" && content.type === "tool_use") {
					if (content.input) this._emit("inputJson", event.delta.partial_json, content.input);
				}
				break;
			}
			case "message_stop":
			...
			case "content_block_stop":
				...
				break;
			case "message_start":
				__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, messageSnapshot, "f");
				break;
		...
		if (this.ended) throw new AnthropicError$1(`stream has ended, this shouldn't happen`);
		const snapshot = __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
		if (!snapshot) throw new AnthropicError$1(`request ended without sending any chunks`);
		__classPrivateFieldSet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, void 0, "f");
		return snapshot;
	}, _PromptCachingBetaMessageStream_accumulateMessage = function _PromptCachingBetaMessageStream_accumulateMessage(event) {
		let snapshot = __classPrivateFieldGet$1(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
		if (event.type === "message_start") {
			if (snapshot) throw new AnthropicError$1(`Unexpected event order, got ${event.type} before receiving "message_stop"`);
			return event.message;
		}
		if (!snapshot) throw new AnthropicError$1(`Unexpected event order, got ${event.type} before "message_start"`);
		switch (event.type) {
			...
			case "content_block_delta": {
				...
				if (snapshotContent?.type === "text" && event.delta.type === "text_delta") snapshotContent.text += event.delta.text;
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/prompt-caching/messages.mjs
...
(function(Messages) {})(Messages$1 || (Messages$1 = {}));
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/prompt-caching/prompt-caching.mjs
...
(function(PromptCaching) {
	PromptCaching.Messages = Messages$1;
...
var MessageStream = class MessageStream {
	...
	*/
	...
	[(_MessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _MessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_listeners = /* @__PURE__ */ new WeakMap(), _MessageStream_ended = /* @__PURE__ */ new WeakMap(), _MessageStream_errored = /* @__PURE__ */ new WeakMap(), _MessageStream_aborted = /* @__PURE__ */ new WeakMap(), _MessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _MessageStream_handleError = /* @__PURE__ */ new WeakMap(), _MessageStream_instances = /* @__PURE__ */ new WeakSet(), _MessageStream_getFinalMessage = function _MessageStream_getFinalMessage() {
		if (this.receivedMessages.length === 0) throw new AnthropicError$1("stream ended without producing a Message with role=assistant");
		return this.receivedMessages.at(-1);
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/messages.mjs
...
var Anthropic = class extends APIClient {
	...
	*/
	constructor({ baseURL = readEnv("ANTHROPIC_BASE_URL"), apiKey = readEnv("ANTHROPIC_API_KEY") ?? null, authToken = readEnv("ANTHROPIC_AUTH_TOKEN") ?? null, ...opts } = {}) {
		...
		if (!options.dangerouslyAllowBrowser && isRunningInBrowser()) throw new AnthropicError$1("It looks like you're running in a browser-like environment.\n\nThis is disabled by default, as it risks exposing your secret API credentials to attackers.\nIf you understand the risks and have appropriate mitigations in place,\nyou can set the `dangerouslyAllowBrowser` option to `true`, e.g.,\n\nnew Anthropic({ apiKey, dangerouslyAllowBrowser: true });\n\nTODO: link!\n");
		super({
	...
	validateHeaders(headers, customHeaders) {
		if (this.apiKey && headers["x-api-key"]) return;
		if (customHeaders["x-api-key"] === null) return;
		if (this.authToken && headers["authorization"]) return;
		if (customHeaders["authorization"] === null) return;
		throw new Error("Could not resolve authentication method. Expected either apiKey or authToken to be set. Or for one of the \"X-Api-Key\" or \"Authorization\" headers to be explicitly omitted");
	}
	authHeaders(opts) {
		const apiKeyAuth = this.apiKeyAuth(opts);
		const bearerAuth = this.bearerAuth(opts);
		if (apiKeyAuth != null && !isEmptyObj(apiKeyAuth)) return apiKeyAuth;
		if (bearerAuth != null && !isEmptyObj(bearerAuth)) return bearerAuth;
		return {};
	}
	apiKeyAuth(opts) {
...
Anthropic.AnthropicError = AnthropicError$1;
...
var { AnthropicError, APIError, APIConnectionError, APIConnectionTimeoutError, APIUserAbortError, NotFoundError, ConflictError, RateLimitError, BadRequestError, AuthenticationError, InternalServerError, PermissionDeniedError, UnprocessableEntityError } = error_exports;
var toFile = toFile$1;
...
//#endregion
export { AI_PROMPT, APIConnectionError, APIConnectionTimeoutError, APIError, APIUserAbortError, Anthropic, AnthropicError, AuthenticationError, BadRequestError, ConflictError, HUMAN_PROMPT, InternalServerError, NotFoundError, PermissionDeniedError, RateLimitError, UnprocessableEntityError, sdk_default as default, fileFromPath, toFile };
